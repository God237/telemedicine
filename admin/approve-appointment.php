...
if (isset($_GET['id'])) {
    ...
    if ($appointment) {
        ...
        if ($stmt->execute()) {
            $_SESSION['success'] = "Appointment #" . $appointment_id . " has been approved successfully. Patient: " . $appointment['patient_name'] . ", Doctor: Dr. " . $appointment['doctor_name'];
            
            // Optional: You can add email notification here
            // mail($appointment['patient_email'], "Appointment Approved", "Your appointment with Dr. " . $appointment['doctor_name'] . " has been approved.");
        } else {
            $_SESSION['error'] = "Failed to approve appointment: " . $conn->error;
        }
    } else {
